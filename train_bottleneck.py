...
def create(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2,
    ...
    model_loss_last = Lambda(YOLOLoss, output_shape=(1,), name='yolo_loss',
                            arguments={
                                'anchors': anchors, 
    ...
    model_loss = Lambda(YOLOLoss, output_shape=(1,), name='yolo_loss',
                        arguments={
                            'anchors': anchors, 
                            'num_classes': num_classes, 
                            'ignore_threshold': 0.5
...
def _data_generator(annotation_lines, batch_size, input_shape, anchors, num_classes, random=True, verbose=False):
    ...
    while True:
        ...
        for b in range(batch_size):
            if i == 0 and random: np.random.shuffle(annotation_lines)
            
            image, box = get_rand_data(annotation_lines[i], input_shape, random=random)
...
def _data_generator_wrapper(annotation_lines, batch_size, input_shape, anchors, num_classes, random=True, verbose=False):
    n = len(annotation_lines)
    
    if n == 0 or batch_size <= 0: return None
    return _data_generator(annotation_lines, batch_size, input_shape, anchors, num_classes, random, verbose)


def _bottleneck_generator(annotation_lines, batch_size, input_shape, anchors, num_classes, bottlenecks):
    n = len(annotation_lines)
    i = 0
    ...
    while True:
        ...
        for b in range(batch_size):
            _, box = get_rand_data(annotation_lines[i], input_shape, random=False, proc_img=False)
